...
const TodoList = () => {
  ...
  // todos 함수의 정의를 useEffect 외부로 옮깁니다.
  const todos = (currentDay, currentMonth, currentYear) => {
    dispatch(actionTodos(currentDay, currentMonth, currentYear));
  };

  useEffect(() => {
    // dispatch 호출을 직접적으로 합니다.
    todos(currentDay, currentMonth, currentYear);
  }, [dispatch, currentDay, currentMonth, currentYear]); // 의존성 배열에서 todos 제거
